...
@Component
class NormalGroupTomatoListener {

    private val log = LoggerFactory.getLogger(this::class.java)

    @KafkaListener(
        topics = [KafkaTopic.TOMATO],
        groupId = KafkaConsumerGroup.TOMATO_NORMAL,
        containerFactory = KafkaConsumerFactory.NORMAL_GROUP,
    )
    fun listen(record: ConsumerRecord<String, String>, ack: Acknowledgment) {
        log.info(
            "[NORMAL-GROUP] partition={}, offset={}, value={}",
            record.partition(),
            record.offset(),
            record.value()
        )
