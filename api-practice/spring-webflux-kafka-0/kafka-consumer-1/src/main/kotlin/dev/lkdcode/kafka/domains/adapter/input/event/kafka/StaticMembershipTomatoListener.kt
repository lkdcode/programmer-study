...
@Component
class StaticMembershipTomatoListener {

    private val log = LoggerFactory.getLogger(this::class.java)

    @KafkaListener(
        topics = [KafkaTopic.TOMATO],
        groupId = KafkaConsumerGroup.TOMATO_STATIC,
        containerFactory = KafkaConsumerFactory.STATIC_MEMBERSHIP,
    )
    fun listen(record: ConsumerRecord<String, String>, ack: Acknowledgment) {
        log.info(
            "[STATIC-MEMBERSHIP] partition={}, offset={}, value={}",
            record.partition(),
            record.offset(),
            record.value()
        )
