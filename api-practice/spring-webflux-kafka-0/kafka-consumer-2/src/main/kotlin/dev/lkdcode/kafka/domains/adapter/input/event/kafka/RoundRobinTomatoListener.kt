...
@Component
class RoundRobinTomatoListener {

    private val log = LoggerFactory.getLogger(this::class.java)

    @KafkaListener(
        topics = [KafkaTopic.TOMATO],
        groupId = KafkaConsumerGroup.TOMATO_ROUND_ROBIN,
        containerFactory = KafkaConsumerFactory.ROUND_ROBIN,
    )
    fun listen(record: ConsumerRecord<String, String>, ack: Acknowledgment) {
        log.info(
            "[ROUND-ROBIN] partition={}, offset={}, value={}",
            record.partition(),
            record.offset(),
            record.value()
        )
