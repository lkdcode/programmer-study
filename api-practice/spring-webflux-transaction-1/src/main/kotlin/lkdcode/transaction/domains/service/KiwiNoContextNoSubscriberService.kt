...
import lkdcode.transaction.domains.repository.KiwiJooqNoContextNoSubscriberRepository
...
@Service
class KiwiNoContextNoSubscriberService(
    private val transactionalOperator: TransactionalOperator,

    private val kiwiR2dbcRepository: KiwiR2dbcRepository,
    private val kiwiJooqNoContextNoSubscriberRepository: KiwiJooqNoContextNoSubscriberRepository,
) {

    fun deleteByNameThenFindAll(name: String): Mono<List<Kiwi>> =
        transactionalOperator.transactional(
            kiwiR2dbcRepository.deleteAllByName(name)
                .thenMany(kiwiJooqNoContextNoSubscriberRepository.findAll())
                .collectList()
        )
}