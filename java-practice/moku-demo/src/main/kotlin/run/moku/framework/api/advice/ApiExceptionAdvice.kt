...
import run.moku.framework.log.logging
...
class ApiExceptionAdvice {
    ...
    fun handleApiException(e: ApiException): ResponseEntity<ApiResponse<String>> {
        logging().info("ApiResponseCode: ${e.getApiResponseCode()}, Message: ${e.message()} Status: ${e.status()}")
    ...
    fun handleConstraintViolationException(e: ConstraintViolationException): ResponseEntity<ApiResponse<List<*>>> {
        val response = e.constraintViolations.map {
            logging().info("Path: ${it.rootBeanClass}, Property: ${it.propertyPath} Invalid value: ${it.invalidValue}, Message: ${it.message}")
    ...
    fun handleHttpRequestMethodNotSupportedException(
        ...
        logging().info("Method: ${e.method} End-Point: ${request.requestURI}")
    ...
    fun handleNoResourceFoundException(
        ...
        logging().info("End-Point: ${e.resourcePath} Message: ${e.message}")
    ...
    fun handleDataAccessException(e: DataAccessException): ResponseEntity<ApiResponse<Unit>> {
        logging().warn("DataAccessException Message: ${e.message}")
    ...
    fun handleRuntimeException(e: RuntimeException): ResponseEntity<ApiResponse<Unit>> {
        logging().error("[Runtime Exception] ${e::class.simpleName}: ${e.message}", e)
    ...
    fun handleException(e: Exception): ResponseEntity<ApiResponse<Unit>> {
        logging().error("[Unhandled Exception] ${e::class.simpleName}: ${e.message}", e)
