CREATE TABLE `tb_users` (
    `id`	      BIGINT        NOT NULL  AUTO_INCREMENT  PRIMARY KEY  COMMENT '식별 인덱스',
    ...
    `is_deleted`  TINYINT(1)	NOT NULL  DEFAULT 0	                   COMMENT '논리 삭제 여부 (\'0\': 정상, \'1\': 삭제)',
    `deleted_at`  DATETIME	    NULL	                               COMMENT '논리 삭제 시간',
    `created_by`  BIGINT	    NOT NULL	                           COMMENT '데이터를 생성한 유저 아이디',
    `updated_by`  BIGINT	    NOT NULL	                           COMMENT '데이터를 수정한 유저 아이디',
