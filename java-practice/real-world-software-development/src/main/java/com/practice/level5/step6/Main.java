...
class Main {
    ...
    public static void main(String[] args) {
        ...
        Engine<People> engine = new Engine<>();

        engine.addRule(RuleBuilder
                .when(people -> people.getName().startsWith("이"))
                .then(people -> System.out.println("[이름이 '이'로 시작] " + people.toString())));

        engine.addRule(RuleBuilder
                .when(people -> people.getAge() >= 30)
                .then(people -> System.out.println("[나이가 30세 이상] " + people.toString())));

        engine.addRule(RuleBuilder
                .when(people -> people.getAddress().equals("강원도"))
                .then(people -> System.out.println("[주소가 강원도] " + people.toString())));
